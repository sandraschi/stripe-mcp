...
# PyInstaller spec — produce stripe-mcp-backend.exe for Tauri NSIS embedding.
# Fleet standard: strip=False, upx=False, noarchive=True (see tauri_nsis_building.md).
# Usage (from repo root):
#   uv run pyinstaller stripe-mcp-backend.spec --distpath dist --clean --noconfirm

block_cipher = None

a = Analysis(
    ["src/stripe_mcp/__main__.py"],
    pathex=["src"],
    binaries=[],
    datas=[("src/stripe_mcp", "stripe_mcp")],
    hiddenimports=[
        "uvicorn.logging",
        "uvicorn.loops",
